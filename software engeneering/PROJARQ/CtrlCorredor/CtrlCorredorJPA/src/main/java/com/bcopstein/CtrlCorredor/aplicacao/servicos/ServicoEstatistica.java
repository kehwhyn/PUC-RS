...
public class ServicoEstatistica {
    ...
    public PerformanceDTO calculaAumentoPerformance(int distancia, int ano) {
        var eventos = this.eventoRepository
                .listaEventos()
                .stream()
                .filter(e -> e.getDistancia() == distancia && e.getData().getAno() == ano)
        ...
        for (int i = 0; i < eventos.size() - 1; i++) {
            ...
            var t1 = e1.getHora().getHoras() * 3600 + e1.getHora().getMinutos() * 60 + e1.getHora().getSegundos();
            var t2 = e2.getHora().getHoras() * 3600 + e2.getHora().getMinutos() * 60 + e2.getHora().getSegundos();
