...
*
*
*  PROGRAMA QUE L� O LOG DO SIMULADOR MARS E GERA UM VHDL COM A
*  INICIALIZA��O DOS BLOCOS DE MEM�RIA
*
...
/**********************************************************************/
int getline2(word, limit, p1) char *word;
FILE *p1;
int limit;
/**********************************************************************/
{
...
//-------------------------------------------------------------------------------
{
    ...
    {
        puts("*******************************************************************************");
        puts("*         PROGRAMA QUE LE DOIS ARQUIVOS: instrucoes e codigo                  *");
        printf("*         uso: %12s <arquivo programa> <arquivo dados> [d]            *\n", argv[0]);
        puts("*         [d] pode ser qualquer caracter, sendo opcional, indica debug        *");
        puts("*         Please report all bugs to {fernando.moraes,ney.calazans}@pucrs.br   * ");
        puts("*******************************************************************************\n");
        return (0);
    }

    // inicializa os strings relativos � mem�ria  (1 BRAM DE LARGURA 32 PARA AS INSTRU��ES E 4 BRAMS DE LARGURA 8 PARA OS DADDOS)
    for (i = 0; i < 64; i++)
    {
        for (j = 0; j < 64; j++)
        {
    ...
    {
        i = 0;                     // aponta para o in�cio da linha
        search_word(line, &i, wd); // wd � ponteiro para pr�ximo token

        if (wd[0] == '0' && wd[1] == 'x') // a linha de programa come�a com "0x" ==> endere�o v�lido
        {
            if (DEBUG)
                printf("Endereco de programa  %10s   ", wd);

            search_word(line, &i, wd); // c�digo

            if (DEBUG)
                printf("com opcode %10s\n", wd);

            // TRANSFERE PARA A MEM�RIA DE DADOS (gera��o da SA�DA)
            for (b = 0; b < 8; b++)
            ...
            if (offset < 0)
            {
                offset = 56; // deslocamento dentro da string relativa � mem�ria - a string vai de 0 a 63
                palavra++;
            }
        }
    }
    ...
    while (getline2(line, TAM, fp))
    {
        ...
        if (wd[0] == '0' && wd[1] == 'x') // a linha de dados come�a com "0x" ==> endere�o v�lido
        {
            if (DEBUG)
            ...
            wd[10] = '\0';                 //  coloca nova linha ap�s o �ltimo d�gito
            sscanf(wd + 6, "%X", &offset); //  transforma para int a partir do quarto caracter
            offset /= 4;                   // divide por 4 para obter o endere�amento em word

            palavra = offset / 32; // a divis�o do endere�o d� o indice na mem�ria

            if (palavra > 63)
                continue; // evita estouro na matriz

            offset -= palavra * 32; // resto

            //     �ndices de  wd:   2 3 4 5 6 7 8 9     exemplo: 12345678

            while (search_word(line, &i, wd)) // l� o restante da linha
                if (wd[0] == '0' && wd[1] == 'x')
                {
                    if (DEBUG)
                        printf("p%d o%d %s\n", palavra, offset, wd);
    ...
    fprintf(out, "--------------------------------------------------------------------------\n");
    fprintf(out, "--  MEM�RIA DE PROGRAMA   - UMA MEM�RIA DE 512 X 32 bits   (16384 BYTES)\n");
    fprintf(out, "--------------------------------------------------------------------------\n");
    ...
    fprintf(out, "\n\n\n--------------------------------------------------------------------------\n");
    fprintf(out, "--  MEM�RIA DE DADOS - 4 mem�rias de 2048 * 8 bits ( 657536 BYTES)    \n");
    fprintf(out, "--\n");
    fprintf(out, "--   supondo na mem�ria de dados:   00 00 00 00 \n");
    fprintf(out, "--                                  10 00 00 AA\n");
    fprintf(out, "--                                  20 00 BB 00\n");
    fprintf(out, "--                                  30 CC 00 00\n");
    fprintf(out, "--\n");
    fprintf(out, "--  As inicializa��es ficam como segue:\n");
    fprintf(out, "--\n");
    ...
    fprintf(out, "          address: in std_logic_vector(10 downto 0);    -- 11 bits - 2048 palavras endere��veis\n");
